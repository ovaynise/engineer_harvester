...
from utils.user_utils import (get_reminder_by_id,
                              add_activity_chat_for_reminder,
                              del_activity_chat_for_reminder)
from filters.chat_types import (
    ChatTypesFilter,
    UserLevelFilter,
    UserLevelRangeFilter,
    security_filters
)
...
async def handle_start_reminder_id(message: types.Message):
    ...
    chat_name = message.chat.title \
        if message.chat.title else message.from_user.first_name
...
async def handle_stop_reminder_id(message: types.Message):
    ...
    await message.delete()
    if len(command_parts) < 2:
        await message.answer("Пожалуйста, укажите номер напоминания"
                               " после команды '/стоп'.")
        return
    reminder_id_str = command_parts[1]
    if not reminder_id_str.isdigit():
    ...
    chat_name = message.chat.title if (
        message.chat.title) else {message.from_user.first_name}
    owner_reminder_id = crypt(message.from_user.id)
    rem_data = {
        'chats_id_active': {}
    }
