...
def security_filters(router: Router, command: str = None, *filters: Filter):
    def decorator(handler: Callable):
        ...
        async def wrapper(message: types.Message):
            ...
            bot_logger.debug(
                f'🟢Фильтры пройдены для пользователя {username}'
            )
...
class UserLevelRangeFilter(Filter):
    ...
    async def __call__(
        ...
        if not (
            ...
            return False
        user_level = await get_user_level(crypt(user_id))
        result = user_level is not None and self.min_level <= user_level <= self.max_level
        if not result:
            await message.answer(f'Для общения с ботом необходимо быть '
