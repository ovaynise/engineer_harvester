...
from config import endpoint_tg_users, endpoint_reminder
...
async def get_reminder_by_id(reminder_id):
    try:
        reminder_response = await api_ov_client.get(f"{endpoint_reminder}{reminder_id}/")
        if reminder_response == None:
            return False
